...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/23 15:58:33 by moboigui          #+#    #+#             */
/*   Updated: 2023/09/10 19:03:26 by hamzaelouar      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_map(t_infos *vars, char **line)
{
	...
	{
		vars->map->grid[i] = garb_col_mlx(vars, ALLOC, (vars->map->width + 1)
				* sizeof(char), NULL);
...
t_map	*load_map(t_infos *vars)
{
	...
	if (!is_surrounded_by_walls(vars->map, vars->player->x, vars->player->y))
		exit_game(vars, "map not surrounded by walls (load map)", 206);
