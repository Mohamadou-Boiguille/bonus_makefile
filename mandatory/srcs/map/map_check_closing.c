...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/24 06:36:31 by moboigui          #+#    #+#             */
/*   Updated: 2023/09/10 19:03:15 by hamzaelouar      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_closed(t_map *map, int visited[128][128], int x, int y)
{
	...
	if (is_closed(map, visited, x - 1, y) && is_closed(map, visited, x + 1, y)
		&& is_closed(map, visited, x, y - 1) && is_closed(map, visited, x, y
			+ 1))
		return (1);
	return (0);
}
...
void	*create_wall_texture(t_infos *vars, int color1, int color2)
{
	...
	wall_data = (int *)mlx_get_data_addr(fb->ptr, &fb->bpp, &fb->size_l,
			&fb->endian);
