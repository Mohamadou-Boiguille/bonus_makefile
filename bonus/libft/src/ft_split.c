...
/*   Updated: 2023/02/02 13:25:20 by moboigui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int	secure_malloc_error(char **arr, int len)
{
	while (len--)
	{
		free(arr[len]);
...
static void	get_array_size(char const *s, char c, int *arrsize)
{
	int	i;
...
static int	fill_array(const char *s, char c, char **result, int arrsize)
{
	int	start;
	int	end;
	int	i;
